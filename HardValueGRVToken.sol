...
contract GravelToken is UpgradeableToken, ReleasableToken {

  event UpdatedTokenInformation(string newName, string newSymbol); 

  /* name of the token */
  string public name = "Gravel"; 

  /* symbol of the token */
  string public symbol = "GRV"; 

  /* token decimals to handle fractions */         
  uint public decimals = 18;

  /* initial token supply */
  uint public totalSupply = 2000000000 * (10 ** decimals);   // 2 billion total supply with 18 decimal places


  function GravelToken() UpgradeableToken(msg.sender) {
  ...
  /* To accept token contract ownership and transfer balances */
  function acceptOwnership() {
  ...
  /* An annual dispense feature to allocate some addresses with Gravel tokens 
  * calculation done using percentage of left tokens with owner
  *  Can be called only by owner
  */
  function dispenseByPercentage(address[] _addresses, uint _percentage) onlyOwner returns (bool ok){
     
     uint _value = safeMul(balances[msg.sender],_percentage) / 100;
     
     for(uint256 i=0; i<_addresses.length; i++){
        balances[_addresses[i]] = safeAdd(balances[_addresses[i]], _value);
        balances[msg.sender] = safeSub(balances[msg.sender], _value);
  ...
  *  Can be called only by owner
  */
  function dispenseByValue(address[] _addresses, uint _value) onlyOwner returns (bool ok){
